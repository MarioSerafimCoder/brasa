...
import { normalizeTvCatalogItem, normalizeTvProfile, normalizeTvProgressMap } from "../server/tv-contract.mjs";
...
import { buildPersonalizedHome, decorateSeriesContinuity, mergeWatchProgress, orderedEpisodes, resolveSeriesContinuation, searchCatalog } from "../server/tv-personalization.mjs";
...
const systemCollectionIds = new Set(["mcu", "dc", "spider-man", "star-wars", "lotr", "harry-potter", "jurassic", "mission-impossible", "science-fiction", "classics", "fast-furious", "pirates-caribbean", "rocky", "pixar", "disney-classics", "dreamworks", "ghibli", "best-picture"]);
...
async function writeServerState(port) {
    ...
    await fs.rename(temporaryStateFile, stateFile);
}
...
async function resolveMedia(mediaKey) {
    const [type, id] = mediaKey.split(":"), stateItem = await mediaStore.get(mediaKey), library = await tvLibraryCache.load();
    let item = null, media = null;
    if (type === "movie") {
        item = library.movieById.get(id);
        if (item?.video) media = { mediaType: "movie", mediaId: id, title: item.title, year: item.year, audience: item.audience, fileSize: item.fileSize };
    }
    if (type === "episode") {
        item = library.episodeById.get(id)?.episode;
        if (item?.video) media = { mediaType: "episode", mediaId: id, title: item.title, audience: item.audience, seasonNumber: item.seasonNumber, episodeNumber: item.episodeNumber, fileSize: item.fileSize };
    }
    if (!item || !media) return null;
    const originalPath = await firstExistingLibraryPath([stateItem?.originalPath, item.originalVideo, item.video]);
    if (stateItem && originalPath && stateItem.originalPath !== originalPath) await mediaStore.update(mediaKey, { originalPath });
    return { ...media, originalPath: originalPath || stateItem?.originalPath || item.originalVideo || item.video };
}

async function firstExistingLibraryPath(candidates) {
    for (const candidate of [...new Set(candidates.filter(Boolean))]) {
        const resolved = (() => { try { return resolvePathInsideLibrary(rootDir, candidate).path; } catch { return ""; } })();
        if (!resolved) continue;
        const stat = await fs.stat(resolved).catch(() => null);
        if (stat?.isFile()) return String(candidate).replace(/\\/g, "/");
    }
    return "";
}
...
async function tvProfiles(device) {
    ...
    return publicProfiles(state.profiles).filter((profile) => !device.allowedProfileIds.length || device.allowedProfileIds.includes(profile.id)).map(normalizeTvProfile);
}
...
async function tvCatalog(device, profileId) {
    ...
    const availableMovies = library.movies.filter((item) => canTvAccess(item, profile));
    const movies = availableMovies.map((item) => tvMovie(item, profileId, profileState));
    const series = library.series.filter((item) => canTvAccess(item, profile)).map((item) => tvSeries(item, profileId, profile, profileState));
    const collections = library.collections.filter((item) => item.banner).map((collection) => ({
        id: collection.id,
        title: collection.title,
        subtitle: collection.subtitle || "",
        banner: collection.banner || "",
        items: resolveTvCollectionMovies(collection, availableMovies)
            .map((movie) => tvMovie(movie, profileId, profileState))
    })).filter((collection) => collection.items.length);
    return { profile: normalizeTvProfile(publicProfile(profile)), movies, series, collections, favorites: profileState.favorites, progress: normalizeTvProgressMap(profileState.progress), updatedAt: new Date().toISOString() };
}
...
async function tvHome(device, profileId) {
    ...
    const state = await readUserState();
    const home = buildPersonalizedHome(catalog, validateProfileState(state.states[profileId] || {}));
    return { ...home, metrics: { generatedMs: Math.round((performance.now() - startedAt) * 10) / 10, payloadBytes: Buffer.byteLength(JSON.stringify(home)) } };
}
...
async function tvPlayback(device, profileId, mediaKey, clientCapabilities = {}, requestedFallback = "", allowPreparation = true, requestedPositionMs = null) {
    ...
    if (type === "series") {
        const parent = library.series.find((series) => String(series.id) === id);
        if (parent && canTvAccess(parent, profile)) {
            const normalizedSeries = tvSeries(parent, profileId, profile, profileState);
            seriesEpisodes = orderedEpisodes(normalizedSeries);
            item = resolveSeriesContinuation(normalizedSeries).episode;
            if (item) mediaKey = item.mediaKey;
        }
    }
    ...
    const base = {
        ...
        resumePosition: requestedPositionMs == null
            ? Math.max(0, Math.round(Number(item.progress?.currentTime || 0) * 1000))
            : Math.max(0, Math.round(Number(requestedPositionMs) || 0)),
        playbackOffset: 0,
...
async function tvSaveSignal(profileId, mediaKey, action, enabled = true) {
    ...
    const current = validateProfileState(state.states[profileId] || {}), now = new Date().toISOString();
    if (["like", "not-for-me"].includes(action)) {
        const ratings = { ...current.ratings }; if (enabled) ratings[mediaKey] = action; else delete ratings[mediaKey]; current.ratings = ratings;
    } else if (action === "hide") current.hiddenSuggestions = enabled ? [...new Set([...current.hiddenSuggestions, mediaKey])] : current.hiddenSuggestions.filter((key) => key !== mediaKey);
    else if (action === "dismiss-continue") { const dismissed = { ...current.continueDismissed }; if (enabled) dismissed[mediaKey] = now; else delete dismissed[mediaKey]; current.continueDismissed = dismissed; }
    else if (action === "mark-watched") {
        const keys = mediaKey.startsWith("series:") ? orderedEpisodes(tvSeries(media, profileId, profile, current)).map((episode) => episode.mediaKey) : [mediaKey];
        for (const key of keys) { const previous = current.progress[key] || {}; current.progress[key] = validateProgress({ ...previous, currentTime: previous.duration || previous.currentTime || 0, percentage: 100, completed: true, updatedAt: now }, key); }
        current.completed = [...new Set([...current.completed, ...keys])];
    } else throw new ValidationError("Ação de personalização inválida.");
    current.activityEvents = [...current.activityEvents, { mediaKey, type: enabled ? action : `undo-${action}`, at: now, seconds: 0 }].slice(-1000);
    current.updatedAt = now; state.states[profileId] = current; await queueUserStateWrite(state); return { action, enabled };
}
async function tvSavePreferences(profileId, input) { const state = await readUserState(); if (!state.profiles.some((item) => item.id === profileId)) throw new NotFoundError("Perfil não encontrado."); const current = validateProfileState(state.states[profileId] || {}); state.states[profileId] = validateProfileState({ ...current, preferences: { ...current.preferences, ...input }, updatedAt: new Date().toISOString() }); await queueUserStateWrite(state); return state.states[profileId].preferences; }
...
function tvMovie(item, profileId, state) { const mediaKey = `movie:${item.id}`; return normalizeTvCatalogItem({ id: String(item.id), mediaKey, type: "movie", title: item.title, originalTitle: item.originalTitle, year: item.year, duration: item.duration, durationMinutes: durationMinutes(item), rating: item.rating, contentRating: item.contentRating, genres: item.genres || [], cast: item.cast || [], directors: item.directors || item.direction || [], themes: item.themes || item.keywords || [], franchise: item.franchise || "", overview: item.overview || "", poster: item.poster || "", backdrop: item.backdrop || item.poster || "", addedAt: item.addedAt || item.fileModifiedAt || item.lastIndexedAt || "", subtitles: item.subtitles || [], favorite: state.favorites.includes(mediaKey) || state.favorites.includes(String(item.id)), reaction: state.ratings[mediaKey] || "", hiddenSuggestion: state.hiddenSuggestions.includes(mediaKey), progress: state.progress[mediaKey] || null, streamUrl: `/api/tv/stream/${encodeURIComponent(mediaKey)}?profileId=${encodeURIComponent(profileId)}` }); }
...
function tvSeries(item, profileId, profile, state) { const mediaKey = `series:${item.id}`; return decorateSeriesContinuity(normalizeTvCatalogItem({ id: String(item.id), mediaKey, type: "series", title: item.title, originalTitle: item.originalTitle, year: item.year, rating: item.rating, contentRating: item.contentRating, genres: item.genres || [], cast: item.cast || [], directors: item.directors || item.direction || [], themes: item.themes || item.keywords || [], franchise: item.franchise || "", overview: item.overview || "", poster: item.poster || "", backdrop: item.backdrop || item.poster || "", addedAt: item.addedAt || "", favorite: state.favorites.includes(mediaKey) || state.favorites.includes(String(item.id)), reaction: state.ratings[mediaKey] || "", hiddenSuggestion: state.hiddenSuggestions.includes(mediaKey), seasons: (item.seasons || []).map((season) => ({ seasonNumber: season.seasonNumber, episodes: (season.episodes || []).filter((episode) => canTvAccess(episode, profile)).map((episode) => tvEpisode(episode, item, profileId, state)) })) })); }
function tvEpisode(episode, series, profileId, state) { const mediaKey = `episode:${episode.id}`; return normalizeTvCatalogItem({ id: String(episode.id), mediaKey, type: "episode", seriesId: String(series.id), title: episode.title, seasonNumber: episode.seasonNumber, episodeNumber: episode.episodeNumber, duration: episode.duration, durationMinutes: durationMinutes(episode), overview: episode.overview || "", poster: episode.thumbnail || series.poster || "", backdrop: episode.backdrop || series.backdrop || "", addedAt: episode.addedAt || "", subtitles: episode.subtitles || [], reaction: state.ratings[mediaKey] || "", progress: state.progress[mediaKey] || null, streamUrl: `/api/tv/stream/${encodeURIComponent(mediaKey)}?profileId=${encodeURIComponent(profileId)}` }); }
function durationMinutes(item) { const direct = Number(item?.durationMinutes); if (Number.isFinite(direct) && direct > 0) return Math.round(direct); const match = String(item?.duration || "").match(/(?:(\d+)\s*h)?\s*(?:(\d+)\s*min)?/i); return match && (match[1] || match[2]) ? Number(match[1] || 0) * 60 + Number(match[2] || 0) : null; }
...
async function handleProfilesApi(request, response, url) {
    ...
    if (resource === "history") {
        if (request.method === "POST") { const entry=validateHistory({ ...await readJsonBody(request), lastWatchedAt: new Date().toISOString() }); state.states[profileId].history=[entry,...state.states[profileId].history.filter((item)=>item.mediaKey!==entry.mediaKey)].slice(0,500); await queueUserStateWrite(state); return sendJson(response,200,{ok:true}); }
...
function validateProgress(input,key){return {mediaType:key.startsWith("episode:")?"episode":"movie",mediaId:String(input.mediaId||key.split(":").slice(1).join(":")),seriesId:String(input.seriesId||""),currentTime:Number(input.currentTime||0),duration:Number(input.duration||0),percentage:Math.min(100,Math.max(0,Number(input.percentage||0))),completed:Boolean(input.completed),updatedAt:preserveWatchTimestamp(input.updatedAt)};}
function validateHistory(input){if(!isValidMediaKey(input.mediaKey))throw new Error("Conteúdo inválido.");return {mediaKey:input.mediaKey,mediaType:input.mediaKey.startsWith("episode:")?"episode":"movie",mediaId:String(input.mediaId||""),title:String(input.title||"").slice(0,160),startedAt:preserveWatchTimestamp(input.startedAt),lastWatchedAt:preserveWatchTimestamp(input.lastWatchedAt),completedAt:String(input.completedAt||"")};}
function isValidProfileId(id){return /^[a-z0-9][a-z0-9-]{1,47}$/.test(id);} function isValidMediaId(id){return /^[a-zA-Z0-9._-]{1,120}$/.test(id);} function isValidMediaKey(key){return /^(movie|episode|series):[a-zA-Z0-9._-]{1,120}$/.test(key);}
