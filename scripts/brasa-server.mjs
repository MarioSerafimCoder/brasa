...
async function tvPlayback(device, profileId, mediaKey, clientCapabilities = {}) {
    ...
    const clientPlan = selectTvPlaybackPlan(probe, clientCapabilities);
    if (clientPlan.mode === "direct") return { ...base, ...codecs, playbackUrl: originalStreamUrl(item.streamUrl), mimeType: contentTypes[extension] || "video/*", preparationStatus: "ready", preparationProgress: 100, playbackMode: "direct", adaptiveReasons: clientPlan.reasons };
...
function originalStreamUrl(value) { return `${value}${String(value).includes("?") ? "&" : "?"}source=original`; }
