...
private fun handleInstallResult(result:InstallResult){if(result.status==PackageInstaller.STATUS_SUCCESS){val version=preferences.pending()?.second.orEmpty();preferences.clearPending();repository.markInstalled();mutable.value=UpdateUiState.Installed(version)}else mutable.value=UpdateUiState.Error(InstallStatusMapper.message(result),currentUpdate())}
...
private fun initialState():UpdateUiState{val pending=preferences.pending();if(pending!=null&&BuildConfig.VERSION_CODE.toLong()>=pending.first){preferences.clearPending();repository.markInstalled();return UpdateUiState.Installed(pending.second)};preferences.consumeInstallResult()?.let{return UpdateUiState.Error(InstallStatusMapper.message(it))};return UpdateUiState.Idle}
