...
@Composable fun UpdateScreen(state:UpdateUiState,onCheck:()->Unit,onDownload:(UpdateManifest)->Unit,onCancel:()->Unit,onInstall:()->Unit,onPermission:()->Unit,onBack:()->Unit,required:Boolean=false){BackHandler{if(state is UpdateUiState.Downloading||state is UpdateUiState.Validating)onCancel()else if(!required||state===UpdateUiState.UpToDate||state is UpdateUiState.Installed)onBack()};val focus=remember{FocusRequester()};LaunchedEffect(state){if(state!==UpdateUiState.Checking&&state !is UpdateUiState.Validating&&state !is UpdateUiState.Installing)runCatching{focus.requestFocus()}};Column(Modifier.fillMaxSize().padding(64.dp),verticalArrangement=Arrangement.Center){BrasaLogo();Spacer(Modifier.height(20.dp));when(state){
...
is UpdateUiState.Available->{Text("Nova versão do BRasa disponível",fontSize=48.sp,color=Color.White);Text("Versão ${state.update.versionName} · ${formatBytes(state.update.sizeBytes)}",fontSize=23.sp,color=Color.LightGray);if(required)Text("Esta atualização precisa ser instalada para continuar.",fontSize=20.sp,color=BrasaOrange);Spacer(Modifier.height(20.dp));Text("Novidades:",fontSize=24.sp,color=Color.White);state.update.releaseNotes.forEach{Text("• $it",fontSize=20.sp,color=Color.White)};Spacer(Modifier.height(24.dp));Row(horizontalArrangement=Arrangement.spacedBy(14.dp)){BrasaButton("Atualizar agora",{onDownload(state.update)},Modifier.focusRequester(focus),style=BrasaButtonStyle.Primary);if(!required)BrasaButton("Lembrar depois",onBack)}}
...
is UpdateUiState.Error->{Text("Não foi possível atualizar",fontSize=40.sp);Text(state.message,fontSize=20.sp,color=BrasaRed);if(required)Text("Verifique o servidor e tente novamente para continuar.",fontSize=20.sp,color=Color.LightGray);Spacer(Modifier.height(24.dp));Row(horizontalArrangement=Arrangement.spacedBy(14.dp)){BrasaButton("Tentar novamente",{state.update?.let(onDownload)?:onCheck()},Modifier.focusRequester(focus));if(!required)BrasaButton("Voltar",onBack)}}
}}}
