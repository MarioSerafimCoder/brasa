...
class BrasaApi(private val http:BrasaHttpClient,private val json:Json,private val capabilities:()->ClientCapabilities={ClientCapabilities()}){
    ...
    suspend fun playback(base:String,profileId:String,mediaKey:String):PlaybackInfo{val value=capabilities();val supported=value.playback;val headers=mapOf("X-BRasa-Playback-Containers" to supported.containers.joinToString(","),"X-BRasa-Video-Codecs" to supported.videoCodecs.joinToString(","),"X-BRasa-Audio-Codecs" to supported.audioCodecs.joinToString(","),"X-BRasa-Hdr-Types" to supported.hdrTypes.joinToString(","),"X-BRasa-Max-Video-Width" to (supported.maxWidth.takeIf{it>0}?:value.screen.width).toString(),"X-BRasa-Max-Video-Height" to (supported.maxHeight.takeIf{it>0}?:value.screen.height).toString());return http.get(base,"/api/v1/tv/playback/${enc(mediaKey)}?profileId=${enc(profileId)}",PlaybackInfo.serializer(),headers=headers)}
