...
class BrasaApi(private val http:BrasaHttpClient,private val json:Json,private val capabilities:()->ClientCapabilities={ClientCapabilities()}){
    ...
    suspend fun scanLibrary(base:String)=http.post(base,"/api/v1/tv/library/scan","{}",LibraryScanStatus.serializer())
    suspend fun libraryScanStatus(base:String)=http.get(base,"/api/v1/tv/library/scan",LibraryScanStatus.serializer())
    ...
    suspend fun signal(base:String,profileId:String,mediaKey:String,action:String,enabled:Boolean)=if(enabled)http.put(base,"/api/v1/tv/profiles/${enc(profileId)}/actions/${enc(action)}/${enc(mediaKey)}","{}",ActionResult.serializer())else http.delete(base,"/api/v1/tv/profiles/${enc(profileId)}/actions/${enc(action)}/${enc(mediaKey)}")
    suspend fun preferences(base:String,profileId:String,autoplayNext:Boolean)=http.put(base,"/api/v1/tv/profiles/${enc(profileId)}/preferences",json.encodeToString(ProfilePreferences.serializer(),ProfilePreferences(autoplayNext)),ProfilePreferences.serializer())
    suspend fun resetPersonalization(base:String,profileId:String)=http.delete(base,"/api/v1/tv/profiles/${enc(profileId)}/personalization")
