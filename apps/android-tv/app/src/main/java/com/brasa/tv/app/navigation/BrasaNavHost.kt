...
@Composable fun BrasaNavHost(container:AppContainer){val nav=rememberNavController();val vm:BrasaViewModel=viewModel(factory=BrasaViewModel.Factory(container.repository));val updateVm:UpdateViewModel=viewModel(factory=UpdateViewModel.Factory(container.updateRepository,container.settings,container.updatePreferences,container.http,container.context,container.apkValidator));val state by vm.state.collectAsState();val updateState by updateVm.state.collectAsState();NavHost(navController=nav,startDestination=Routes.Server){
...
composable(Routes.Home){LaunchedEffect(state.profile?.id){if(state.profile!=null)updateVm.check(false)};LaunchedEffect(updateState){if(updateState is UpdateUiState.Available&&nav.currentDestination?.route==Routes.Home)nav.navigate(Routes.Update)};HomeScreen(state,onItem={vm.select(it);nav.navigate(Routes.Details)},onSearch={nav.navigate(Routes.Search)},onSettings={nav.navigate(Routes.Settings)},onRefresh=vm::refreshHome)}
...
composable(Routes.Update){UpdateScreen(updateState,onCheck={updateVm.check(true)},onDownload=updateVm::download,onCancel=updateVm::cancel,onInstall={},onBack={updateVm.defer();nav.popBackStack()})}
}}
