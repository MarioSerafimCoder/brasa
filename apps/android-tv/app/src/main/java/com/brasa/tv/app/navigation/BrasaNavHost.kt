...
object Routes{const val Server="server";const val Pairing="pairing";const val Profiles="profiles";const val ProfilePin="profile-pin";const val Home="home";const val Catalog="catalog";const val Search="search";const val Details="details";const val Player="player";const val Settings="settings";const val Update="update"}
...
composable(Routes.Settings){SettingsScreen(state,lastUpdateCheckAt=updateVm.lastCheckAt(),onUpdates={updateVm.check(true);nav.navigate(Routes.Update)},onProfiles={nav.navigate(Routes.Profiles)},onClearCache=vm::clearPlaybackCache,onLoadCache=vm::refreshCacheUsage,onForget={vm.forget{nav.navigate(Routes.Server){popUpTo(0)}}},onBack={nav.popBackStack()})}
