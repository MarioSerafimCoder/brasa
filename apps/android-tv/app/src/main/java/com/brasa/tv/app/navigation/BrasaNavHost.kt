...
composable(Routes.Player){PlayerScreen(state,container,onProgress=vm::saveProgress,onPlaybackFallback=vm::fallbackPlayback,onNext={vm.select(it);vm.loadPlayback(it){}},onBack={nav.popBackStack()})}
composable(Routes.Settings){SettingsScreen(state,container.settings,lastUpdateCheckAt=updateVm.lastCheckAt(),onUpdates={updateVm.check(true);nav.navigate(Routes.Update)},onNetworkDiagnostics={nav.navigate(Routes.NetworkDiagnostics)},onProfiles={nav.navigate(Routes.Profiles)},onClearCache=vm::clearPlaybackCache,onLoadCache=vm::refreshCacheUsage,onForget={vm.forget{nav.navigate(Routes.Server){popUpTo(0)}}},onBack={nav.popBackStack()})}
